...
async function initDatabase() {
  try {
    ...
    const termResult = await pool.query('SELECT COUNT(*) FROM it_terms');
    
    const userCount = parseInt(userResult.rows[0].count);
    const questionCount = parseInt(questionResult.rows[0].count);
    const termCount = parseInt(termResult.rows[0].count);
    ...
    if (userCount === 0) {
      ...
      console.log(`📊 データベースには既に ${userCount} 人のユーザーがいます`);
    }
    
    if (questionCount === 0) {
      console.log('📝 基本情報技術者サンプル問題を作成中...');
      await pool.query(`
      ...
      `);
      console.log('✅ 基本情報技術者サンプル問題が正常に作成されました');
    } else {
      console.log(`📊 データベースには既に ${questionCount} 個の問題があります`);
    }
    
    if (termCount === 0) {
      console.log('📝 IT用語サンプルを作成中...');
      await pool.query(`
      ...
      `);
      console.log('✅ IT用語サンプルが正常に作成されました');
    } else {
      console.log(`📊 データベースには既に ${termCount} 個のIT用語があります`);
    }
